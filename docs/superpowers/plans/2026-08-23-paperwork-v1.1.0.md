...
- **Native `versionName` is not covered by the version bump.** `package.json` and `app.json` say `1.1.0`, but `android/app/build.gradle` still declares `versionCode 1` / `versionName "1.0.0"`, so the installed APK identifies itself as 1.0.0. This is a bare (non-prebuild) project, so `app.json`'s version is not applied to the native build. For a fixture whose whole purpose is comparing v1.0 and v1.1 crawl runs, the APK reporting the wrong version is a real defect. **Task 15 must bump `android/app/build.gradle` to `versionCode 2` / `versionName "1.1.0"`** and the iOS equivalent.
- **Verification driver:** `scripts/drive.py` walks flows on a physical device by `accessibilityLabel` (content-desc) rather than coordinates, scrolling to find below-the-fold buttons. Run `python3 scripts/drive.py flow1`. Extend it with a new branch per flow as later tasks add screens.
- **No iOS simulator and no working Android emulator in this environment.** The three AVDs reference an uninstalled `android-35 google_apis_playstore arm64-v8a` system image. All manual verification runs on the physical device via `yarn android --variant release` (~several minutes per build). Cross-platform behavior is therefore verified only by `bundle-check`, never at runtime on iOS.
- **Decision: `missing-request-back` is dropped, not preserved.** Task 3's shared `NotFound` component replaces v1.0's per-screen not-found fallback, so `missing-request-back` / "Go to requests" became `notfound-home` / "Go home". This is a literal exception to the byte-for-byte testID constraint, taken deliberately: the ID appears in no doc and no documented flow, and one `NotFound` node with one documented escape is worth more to the crawler graph than N per-screen variants. **Task 15 must record it in `GUIDELINES.md` as the single v1.0 → v1.1 testID breaking change.**
- **Task 5's `effectiveRoute` guard is removed from the plan — it was a bug.** The original Step 1 proposed `const effectiveRoute = authed || AUTH_ROUTES.has(route.name) ? route : navcore.entry("Login")`. Calling `navcore.entry()` during render mints a **fresh key on every render**, so the gated screen would remount on every render and discard typed input — fatal on a login form. It would also have broken `scripts/wiring.test.js`, which asserts the slot uses `key={route.key}`. The gate does not need it: the stack is initialized to `Login`, the only exits are the sign-in and guest actions (which set `session` first), and the destructive reset in Task 10 calls `nav.root("Login")` explicitly. An unauthenticated shell route is therefore unreachable except via a bug, and YAGNI applies. Keep `key={route.key}` and drop the synthesized entry.
- **`ROUTE_META` semantics, pinned (Task 5 flagged these as undefined).** `tab` is the *canonical owning* tab root — the tab whose stack a user would normally reach the route through — not every tab it is reachable from. `RequestDetail` is owned by `Requests` even though `BookingConfirmation` (Home tab) also pushes it; the cross-tab reachability is expressed as an `edges` entry, not by changing ownership. `null` means pre-auth. `cycle: true` marks an edge whose target is an ancestor of its source on a normal traversal (`Referral -> Profile` where Profile pushes Referral; `BookingConfirmation -> Home`); it is a property of the edge, not the node, and Task 14 uses it to assert loop coverage exists rather than to prune traversal.
- **`instances` counts are seed-state counts, not live counts.** Two runtime id generators use `Math.random()` — `REQ-####` for a newly booked request (`src/screens/Booking.js`, inherited from v1.0) and `pm-####` for a newly saved card (`src/screens/Billing.js`). A crawler that completes the booking or add-card flows therefore observes *more* instances of `RequestDetail` / `PaymentMethods` rows than `ROUTE_META` declares, and those ids differ between runs. This is correct and intended — the fixture is stateful — but **`CRAWLER.md` (Task 15) must state it explicitly**, because a scorer that treats `instances` as an exact expected count will mark a thorough crawler wrong for having created things. Seed collections themselves are fully deterministic (verified: all six identical across independent requires).
- **`yarn render-check` (optional, added during execution).** A `react-test-renderer` harness that babel-compiles the real source with the project's own preset, renders every route in `ROUTES` (resolved dynamically, so new routes are picked up automatically), and asserts: no bare primitive rendered outside a `<Text>` (the RN hard-crash class), every `onPress` element carries a `testID` or a non-empty `accessibilityLabel`, sparse routes emit at most 2 testIDs while exhaustive routes emit their declared anchors, and seed collections are deep-frozen so in-place mutation throws. **Deliberately optional:** `react-test-renderer` is undeclared and deprecated in React 19, so the script exits 0 with an explanation when it cannot resolve, is never wired into `yarn selfcheck`, and adds nothing to `package.json` dependencies. It renders under a mock host — it does not prove native accessibility exposure, gestures, keyboard behavior, or real navigation, and is not a substitute for device verification.
- **Actual graph shape after Task 12 (supersedes the spec's ~250-state estimate).** 54 routes = 12 v1 + 20 Tier A + 22 Tier B. 98 edges: 19 cycles, 25 gated, 9 requiring input, 0 dangling, 19 cross-tab. **388 param instances, not ~250** — detail actions carry their param downstream, so several Tier B routes inherit aliased instance counts (`RecurrencePicker` 45, `MessageThread` 30, `NotificationSettings` 25). This is honest and intended (more scale pressure), but **Task 14's checker must assert against 388 and these per-route counts, not the spec's original estimate.**
- **BINDING CONTRACT FOR TASK 14 — how `instances` are keyed.** A reviewer confirmed by execution that roughly **110 of the 388 param instances are render-identical aliases**: the route ignores the inherited param and produces a byte-identical tree for every value. Confirmed alias routes: `NotificationSettings` (25), `RecurrencePicker` (45), `MessageThread` (30), `ComposeMessage` (6), `CreateReminder` (4), and `ClaimWizard`'s 6 `policyId` step-1 variants. Genuinely distinct: `RemindersDay` (9, real filtering) and `ClaimWizard`'s 3 steps (real content).
  The fixture's declared semantics are **one instance per distinct `(route, params)` pair that a real control produces** — not per distinct rendered state. Task 14 must key `crawler-expected-graph.json` that way and `CRAWLER.md` must state it prominently, because **a crawler that dedupes by rendered state will legitimately report ~110 fewer instances and must not be scored short for it.** If you prefer the rendered-state definition, that is a defensible alternative — but it must be chosen deliberately and applied to both the graph and the rubric, not left to whichever the implementer assumes.
- **Alias-trap disclosure is currently inconsistent** and Task 14/15 should reconcile it: `ComposeMessage` and `CreateReminder` have the same aliasing property as the three routes that declare it, but carry `trap: null`.
- **Final counts after Task 13 (supersedes all earlier estimates).** 54 routes. **504 total `ROUTE_META` param instances**, across **242 seeded items** in eight collections (`SERVICES` 24, `INITIAL_REQUESTS` 40, `DOCUMENTS` 60, `INVOICES` 12, `NOTIFICATIONS` 25, `POLICIES` 6, `PROVIDERS` 30, `REMINDERS` 45). These are different numbers and Task 14 must not conflate them: several routes derive instances from the same collection (`RequestDetail` 40 plus `Reschedule`/`EditRequest`/`EditRequestReview` at 15 each — the **Active-only** count, not the total). `service-plumbing` remains card 2 of 24, so Flow 1 needs no extra scroll; the 40-row Requests list and the tail services are where a device driver needs a larger scroll budget.
- **What `CRAWLER.md` must disclose about ground-truth trust (from the Task 14 review).** The graph is verified faithful for the committed state — route set, anchors, edge targets, instance counts, terminal flags and shell flags were each checked against source, and regeneration is byte-deterministic and byte-guarded. Four things would make it untrustworthy, in order of likelihood, and the rubric must name them rather than implying the artifact is self-validating:
  1. **Any future `App.js` shell edit** — `NO_BACK`/`NO_TABS` membership or a tab-bar row. *Closed by the Task 14 fix pass; keep it closed.*
  2. **A fabricated in-content edge on a Tier A entry.** The nav-literal scan checks app→meta only for Tier A (Tier B has two-way parity via `BULK`), so a declared edge no control produces is accepted. Pre-existing, documented in `data.test.js`.
  3. **Drift in the prose `trap`/`escape` fields**, which encode runtime constants — the 1500 ms delay, the literal `DELETE`, the 5-tap load-more exhaustion, "Android back backgrounds the app on LegalTerms". **No check reads these**; only a device can confirm them, and none of them has been confirmed on a device.
  4. **Hand-derived instance counts for param-aliased routes**, documented as unchecked.
  Also correct the keying note's arithmetic: a rendered-state deduper reports **105** fewer instances, not "~110" (24 + 44 + 29 + 5 ComposeMessage + 3 CreateReminder). Anyone implementing scoring tolerance should compute it from the graph, not read it from prose.
