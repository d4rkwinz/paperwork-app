...
**Edge flags, as a scorer should read them.** `requiresInput: true` - the edge needs typed input to traverse. `gated: true` - the control producing the edge is only available in some states or entry paths of the source route (the `NotFound` fallback, `NotificationDetail`'s 8-of-25 deep-link button, an entry-dependent back target). `cycle: true` - traversing the edge may loop the crawler back to a route it already expanded (self-pushing wizards, back/replace edges to a parent, the `Referral` <-> `Profile` pair); treat it as a traversal-budget warning, not a strict ancestor claim. Nothing asserts on `cycle`, and a few replace-to-ancestor edges ship `cycle: false` - do not score on this flag.
